...
def run_training(model, batcher, sess_context_manager, sv, summary_writer):
  ...
  with sess_context_manager as sess:
    for _ in range(FLAGS.max_train_iter): # repeats until interrupted
      ...
      recalls = np.zeros((9))
      sent_nums = np.zeros((9))
      avg_ps = []
      avg_rs = []
      for i in range(FLAGS.batch_size):
        sent_num, ps, rs, avg_p, avg_r = util.get_select_AP(batch.original_articles_sents[i], \
                                                            results['probs'][i, :], \
                                                            batch.original_extract_sent_ids[i])
      ...
      tf.logging.info("avg num of sentences: " + str(sent_nums/float(FLAGS.batch_size)))
      tf.logging.info('precision on all thres: ' + str(precisions/FLAGS.batch_size))
      tf.logging.info('recalls on all thres: ' + str(recalls/FLAGS.batch_size))
      tf.logging.info('avg precision: %f', sum(avg_ps)/FLAGS.batch_size)
      tf.logging.info('avg recall: %f', sum(avg_rs)/FLAGS.batch_size)
