...
class Example(object):
  ...
  def __init__(self, article_sentences, extract_ids, abstract_sentences, vocab, hps):
    ...
    for idx, sent in enumerate(article_sentences):
      ...
      self.sent_lens.append(len(sent_words))
  ...
  def pad_encoder_input(self, max_art_len, max_enc_len, pad_id):
    """For rewriter, pad the encoder input sequence with pad_id up to max_len."""
    while len(self.enc_input) < max_enc_len:
      self.enc_input.append(pad_id)
      self.enc_input_sent_ids.append(-1)
    while self.art_len < max_art_len:
...
class Batch(object):
  ...
  def __init__(self, example_list, hps, vocab):
    ...
    if hps.model in ['rewriter', 'end2end']:
      self.init_decoder_seq(example_list, hps) # initialize the input and targets for the rewriter decoder
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    for ex in example_list:
      ex.pad_encoder_input(hps.max_art_len, max_enc_seq_len, self.pad_id)
