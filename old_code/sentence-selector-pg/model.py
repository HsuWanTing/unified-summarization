...
class SentenceSelector(object):
  ...
  def _get_rewards(self, actions, batch):
    ...
    for i in range(hps.batch_size):
      ...
      gt_hit = [idx for idx in range(max_art_len) if idx in batch.original_extract_sent_ids[i] and actions[i, idx] == 1.0]
      gt_recalls.append(len(gt_hit) / float(len(batch.original_extract_sent_ids[i])))
