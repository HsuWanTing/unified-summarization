...
class Example(object):
  ...
  def __init__(self, article_sentences, extract_sent_ids, abstract_sentences, vocab, hps):
    ...
    self.extract_sent_ids = extract_sent_ids
    '''
  ...
  def pad_article(self, max_art_len, max_sent_len, pad_id):
    """Pad the article with pad_id up to max_art_len sentences and 
       max_sent_len words for each sentence."""
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (article, abstract, extract, extract_ids) = input_gen.next() # read the next example from file. article and abstract are both strings.
      ...
      article_sentences = [sent.strip() for sent in data.abstract2sents(article)]
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      if len(article_text)==0: # See https://github.com/abisee/pointer-generator/issues/1
        ...
        yield (article_text, abstract_text, extract_text, extract_ids_str)
