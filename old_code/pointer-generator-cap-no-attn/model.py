...
class SummarizationModel(object):
  ...
  def _caption_encoder(self, caption_inputs, seq_len):
    with tf.variable_scope('caption'):
      cell_fw = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim, initializer=self.rand_unif_init, state_is_tuple=True)
      cell_bw = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim, initializer=self.rand_unif_init, state_is_tuple=True)
      (caption_outputs, (fw_cap_st, bw_cap_st)) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, caption_inputs, dtype=tf.float32, sequence_length=seq_len, swap_memory=True)
      caption_outputs = tf.concat(axis=2, values=caption_outputs)
    return caption_outputs, fw_cap_st, bw_cap_st   
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      cap_outputs, fw_cap_st, bw_cap_st = self._caption_encoder(emb_cap_inputs, self._cap_lens)
      ...
      self._cap_state = self._reduce_states(fw_cap_st, bw_cap_st, 'reduce_cap_final_st')
      self._cap_state = tf.concat(self._cap_state, 1)
  ...
  def run_encoder(self, sess, batch):
    ...
    (enc_states, cap_states, dec_in_state, cap_state, global_step) = sess.run([self._enc_states, self._cap_states, self._dec_in_state, self._cap_state, self.global_step], feed_dict) # run the encoder

    # dec_in_state is LSTMStateTuple shape ([batch_size,hidden_dim],[batch_size,hidden_dim])
    # Given that the batch is a single example repeated, dec_in_state is identical across the batch so we just take the top row.
    dec_in_state = tf.contrib.rnn.LSTMStateTuple(dec_in_state.c[0], dec_in_state.h[0])
    #cap_state = tf.contrib.rnn.LSTMStateTuple(cap_state.c[0], cap_state.h[0])
    return enc_states, dec_in_state, cap_states, cap_state


  def decode_onestep(self, sess, batch, latest_tokens, enc_states, cap_state, dec_init_states, prev_context, prev_coverage):
