...
def setup_training(model, batcher):
  ...
  with default_device:
    ...
    if FLAGS.model == 'rewriter':  # load a pretrained selector and train a new rewriter
      if FLAGS.pretrained_selector_path:
        ...
        #rewriter_vars = [param for param in params if "seq2seq" in param.name]
        #rewriter_vars += [param for param in params if 'global_step' in param.name]
        uninitialized_vars = [param for param in params if param not in selector_vars]
        selector_saver = tf.train.Saver(selector_vars)  # saver for loading pretrained selector
  ...
  if FLAGS.pretrained_selector_path or FLAGS.pretrained_rewriter_path:
