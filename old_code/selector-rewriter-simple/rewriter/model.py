...
class Rewriter(object):
  ...
  def __init__(self, hps, vocab, selector_model):
    self._hps = hps
    self._vocab = vocab
  ...
  def _add_decoder_one_step(self, inputs):
    ...
    output, out_state, attn_dist, context_vector, p_gen, coverage = attention_decoder_one_step(\
                                                ...
                                                sent_probs=self._sent_probs, \
                                                enc_sent_id_mask=self._selector._enc_sent_id_mask)
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      self._enc_states, self._dec_in_state, self._sent_probs = self._selector._add_sent_selector()
      if hps.model == 'rewriter':
        self._enc_states = tf.stop_gradients(self._enc_states)
        self._dec_in_state = tf.stop_gradients(self.dec_in_state)
        self._sent_probs = tf.stop_gradients(self._sent_probs)
      ...
      with tf.variable_scope('embedding'):
        ...
        #                                  initializer=self.trunc_norm_init)
        self.embedding = self._selector.embedding
  ...
  def _add_train_op(self):
    ...
    if hps.model == 'end2end' and hps.selector_loss_in_end2end:
  ...
  def build_graph(self):
    ...
    t0 = time.time()
    if self._hps.model == 'end2end' and self._hps.selector_loss_in_end2end and self._hps.loss == 'PG':
  ...
  def run_train_step(self, sess, batch):
    ...
    if hps.model == 'end2end' and hps.selector_loss_in_end2end and hps.loss == 'PG':
  ...
  def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, \
                     prev_context, prev_coverage, sent_probs=None):
    ...
    if self._hps.model == 'end2end':
      feed[self._sent_probs] = sent_probs
      feed[self._selector._enc_sent_id_mask] = batch.enc_sent_id_mask
