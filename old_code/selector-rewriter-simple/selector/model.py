  ...
  def _add_sent_selector(self):
    ...
    with tf.variable_scope('SentSelector'):
      ...
      with tf.variable_scope('embedding'):
        ...
        word_enc_inputs = tf.nn.embedding_lookup(self.embedding, self._enc_batch)  # tensor with shape (batch_size, enc_len, emb_size)
      ...
      word_enc_outputs, fw_st, bw_st = self._add_encoder(word_enc_inputs, self._enc_lens, name='word_level_encoder') # (batch_size, enc_len, hidden_dim*2)

      # Our encoder is bidirectional and our decoder is unidirectional so we need to reduce the final encoder hidden state to the right size to be the initial decoder hidden state
      dec_in_state = self._reduce_states(fw_st, bw_st)  # this is for the decoder
      ...
      batch_nums_tile = tf.tile(batch_nums, [1, tf.shape(self._enc_sent_id_mask)[1]]) # shape (batch_size, enc_len)
      indices = tf.stack( (batch_nums_tile, self._enc_sent_id_mask), axis=2) # shape (batch_size, enc_len, 2)
      shape = [hps.batch_size, hps.max_art_len, hps.hidden_dim_selector*2]
      sent_enc_inputs = tf.scatter_nd(indices, word_enc_outputs, shape) # shape (batch_size, max_art_len, hidden_dim*2)

      sent_lens_float = tf.expand_dims(tf.to_float(self._sent_lens), 1) # shape (batch_size, max_art_len, 1)
      self.sent_lens_float = tf.where(sent_lens_float > 0.0, sent_lens_float, tf.ones(sent_lens_float.get_shape().as_list()))
      sent_enc_inputs = sent_enc_inputs / self.sent_lens_float # (batch_size, max_art_len, hidden_dim*2)
      #sent_enc_inputs = tf.reshape(sent_enc_inputs, [hps.batch_size, -1, hps.hidden_dim_selector*2]) # (batch_size, max_art_len, hidden_dim*2)
      sent_enc_outputs, _, _ = self._add_encoder(sent_enc_inputs, self._art_lens, name='sent_level_encoder') # (batch_size, max_art_len, hidden_dim*2)

      # Get each sentence representation and the document representation.
      sent_feats = tf.contrib.layers.fully_connected(sent_enc_outputs, hps.hidden_dim_selector, activation_fn=tf.tanh) # (batch_size, max_art_len, hidden_dim)
      art_padding_mask = tf.expand_dims(self._art_padding_mask, 2) # (batch_size, max_art_len, 1)
      art_feats = tf.reduce_sum(sent_enc_outputs * art_padding_mask, axis=1) / tf.reduce_sum(art_padding_mask, axis=1) # (batch_size, hidden_dim)
      ...
      if self._graph_mode == 'compute_loss':
        with tf.variable_scope('loss'):
          ...
          if hps.loss == 'PG' and hps.regu_ratio_wt > 0.0:
            avg_prob = tf.reduce_sum(self.probs * self._art_padding_mask, 1) / tf.reduce_sum(self._art_padding_mask, 1)
            avg_prob = tf.reduce_mean(avg_prob)
            self._ratio_loss = tf.square(avg_prob - hps.regu_ratio)
            tf.summary.scalar('ratio_loss', self._ratio_loss)
            self._total_loss = self._loss + hps.regu_ratio_wt * self._ratio_loss
            tf.summary.scalar('total_loss', self._total_loss)

      return word_enc_outputs, dec_in_state, self.probs
