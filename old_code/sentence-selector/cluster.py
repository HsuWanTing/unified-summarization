...
if __name__ == "__main__":
    ...
    pk.dump(save_data, open(os.path.join('log', 'gt_diversity_%dcluster.pkl' % cluster_num), 'wb'))

    avg_diversity = sum(diversity) / len(diversity)
    plt.figure(figsize=(10, 5))
    plt.hist(np.array(diversity), bins=np.arange(0.2, 1.0001, 0.05))
    plt.xticks(np.arange(0.2, 1.0001, 0.05))
    plt.grid(True)
    plt.xlabel("diversity")
    plt.ylabel("number of articles")
    plt.title("Diversity of GT selected sentences (%d clusters)" % cluster_num)
    plt.savefig(os.path.join('log', 'gt_diversity_%dcluster.png' % cluster_num))
    plt.clf()
    pdb.set_trace()
