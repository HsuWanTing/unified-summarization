...
class BeamSearchDecoder(object):
  ...
  def __init__(self, model, batcher, vocab, ckpt_path):
    ...
    ckpt_path = util.load_ckpt(self._saver, self._sess, ckpt_path)
    ...
    if FLAGS.single_pass:
      ...
      ckpt_name = "ckpt-" + ckpt_path.split('-')[-1] # this is something of the form "ckpt-123456"
      self._decode_dir = os.path.join(FLAGS.log_root, get_decode_dir_name(ckpt_name) + '_random')
  ...
  def decode(self):
    ...
    while True:
      ...
      if FLAGS.single_pass:
        ...
        self.save_result(original_article, original_abstract_sents, decoded_sents, counter)
        ...
        self.write_for_attnvis(article_withunks, abstract_withunks, decoded_words, best_hyp.attn_dists, \
                               best_hyp.p_gens, best_hyp.log_probs, counter) # write info to .json file for visualization tool
  ...
  def save_result(self, article_str, reference_sents, decoded_sents, index):
  ...
  def write_for_attnvis(self, article, abstract, decoded_words, attn_dists, p_gens, log_probs, count=None):
