...
class SummarizationModel(object):
  ...
  def _add_placeholders(self):
    ...
    if hps.training_method == 'PG':
      ...
      self.rouge_L_sample_mean = tf.placeholder("float32", [], name='rouge_L_sample')
      self.copy_rate_sample_mean = tf.placeholder("float32", [], name='copy_rate_sample')
      self.rouge_L_argmax_mean = tf.placeholder("float32", [], name='rouge_L_argmax')
      self.copy_rate_argmax_mean = tf.placeholder("float32", [], name='copy_rate_argmax')
  ...
  def _add_decoder(self, dec_inputs, predict_method='sample', reuse=False):
    ...
    if hps.training_method == 'PG': # train and eval mode for policy gradient
      ...
      return predict_words, log_dists, attn_dists, p_gens
      ...
      if hps.mode in ['train', 'eval']:
        with tf.variable_scope('loss'):
          if hps.training_method == 'TF': 
            ...
            self._loss = -tf.reduce_sum(action_log_probs * self.advantages) / tf.reduce_sum(predict_masks)
            ...
            tf.summary.scalar('rouge_L_sample', self.rouge_L_sample_mean)
            tf.summary.scalar('copy_rate_sample', self.copy_rate_sample_mean)
            tf.summary.scalar('rouge_L_argmax', self.rouge_L_argmax_mean)
            tf.summary.scalar('copy_rate_argmax', self.copy_rate_argmax_mean)
  ...
  def _get_actions_rewards(self, sess, feed_dict, batch):
    ...
    for i in range(hps.batch_size):
      # Get rewards
      gen_sents, score_sample, rouge_L_s, copy_rate_s = \
                                   _summary_score(self._vocab, pred_words_sample[i], \
                                        batch.original_abstracts_sents[i], \
                                        batch.original_articles[i], batch.art_oovs[i], \
                                        hps.reward_ratio)
      rewards.append(score_sample)
      rouge_L_sample.append(rouge_L_s)
      copy_rate_sample.append(copy_rate_s)
      if i == 0:
      ...
      # Get advantages
      if hps.use_baseline:
        gen_sents, score_argmax, rouge_L_a, copy_rate_a = \
                                    _summary_score(self._vocab, pred_words_argmax[i], \
                                      batch.original_abstracts_sents[i], \
                                      batch.original_articles[i], batch.art_oovs[i], \
                                      hps.reward_ratio)
        advantages.append(score_sample - score_argmax) # len batch_size
        rouge_L_argmax.append(rouge_L_a)
        copy_rate_argmax.append(copy_rate_a)
        if i == 0:
    ...
    advantages = np.tile(np.array(advantages).reshape(-1, 1), (1, hps.max_dec_steps))
    rewards = np.tile(np.array(rewards).reshape(-1, 1), (1, hps.max_dec_steps))

    rouge_L_sample_mean = sum(rouge_L_sample) / hps.batch_size
    copy_rate_sample_mean = sum(copy_rate_sample) / hps.batch_size
    rouge_L_argmax_mean = sum(rouge_L_argmax) / hps.batch_size
    copy_rate_argmax_mean = sum(copy_rate_argmax) / hps.batch_size
    ...
    return pred_words_sample, rewards, advantages, \
           rouge_L_sample_mean, copy_rate_sample_mean, rouge_L_argmax_mean, copy_rate_argmax_mean
  ...
  def run_train_step(self, sess, batch):
    ...
    if hps.training_method == 'PG':
      actions, rewards, advantages, rouge_L_s, copy_rate_s, rouge_L_a, copy_rate_a = \
                                            self._get_actions_rewards(sess, feed_dict, batch)
      ...
      feed_dict[self.rouge_L_sample_mean] = rouge_L_s
      feed_dict[self.copy_rate_sample_mean] = copy_rate_s
      feed_dict[self.rouge_L_argmax_mean] = rouge_L_a
      feed_dict[self.copy_rate_argmax_mean] = copy_rate_a
...
def _summary_score(vocab, words_idx, ref_sents, article, art_oovs, alpha):
  ...
  score, rouge_l_score, copy_score = reward_criterion.reward(gen_sents, ref_sents, article, alpha)
  return gen_sents, score, rouge_l_score, copy_score
