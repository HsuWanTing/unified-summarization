...
def copy_rate_rouge_n(summ, article,n):
    '''
    summ: summary that need to be calculated, a list of sentences
    article: article of the summary, a single string
    '''
    rouge_f, rouge_p, rouge_r = rouge_quick.rouge_n(summ,[article], n)
    return rouge_p


def reward(summ, ref, article, alpha):
    rouges = rouge_scores_quick(summ, ref, metric=['rouge-l'])
    copy_score1 = copy_rate_rouge_n(summ, article,4)
    copy_score2 = copy_rate_rouge_n(summ, article,5)
    copy_score3 = copy_rate_rouge_n(summ, article,6)
    copy_score = ((copy_score1+copy_score2+copy_score3)/3)
    #reward = rouges['rouge-1-f'] + rouges['rouge-2-f'] + rouges['rouge-l-f'] - copy_score
    reward = rouges['rouge-l-f'] - alpha * max((copy_score - 0.155), 0)
    #pdb.set_trace()
    return reward, rouges['rouge-l-f'], copy_score
