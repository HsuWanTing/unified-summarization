...
tf.app.flags.DEFINE_float('reward_ratio', 0.01, 'learning rate')
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer):
  ...
  if FLAGS.coverage:
    ckpt_path = os.path.join(FLAGS.log_root, "train", "model.ckpt_" + FLAGS.training_method + "_cov")
  else:
    ckpt_path = os.path.join(FLAGS.log_root, "train", "model.ckpt_" + FLAGS.training_method)
  t_total=0.0
  with sess_context_manager as sess:
    for _ in range(FLAGS.max_train_iter): # repeats until interrupted
      ...
      if int(t_total) % 600==0:
	print 'total seconds=: ', t_total
...
def main(unused_argv):
  ...
  hparam_list = ['training_method', 'use_baseline', 'mode', 'reward_ratio', 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 'max_grad_norm', 'hidden_dim', 'emb_dim', 'batch_size', 'max_dec_steps', 'max_enc_steps', 'coverage', 'cov_loss_wt', 'pointer_gen']
  hps_dict = {}
