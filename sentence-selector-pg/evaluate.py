...
class SelectorEval(object):
  ...
  def evaluate(self):
    ...
    while True:
      ...
      original_gt_sents = [original_article_sents[idx] for idx in original_selected_ids]
      #pdb.set_trace()

      if FLAGS.single_pass:
        if FLAGS.eval_gt_rouge:
          self.write_for_rouge(original_abstract_sents, original_gt_sents, counter) # write ref summary and decoded summary to file, to eval with pyrouge later
          counter += 1
          continue
      ...
      selected_sents, selected_ids, p, r, acc, ratio = util.get_select_accuracy_one_thres(original_article_sents, \
      ...
      if FLAGS.single_pass:
        ...
        if FLAGS.save_pkl:
          self.save_result(original_article_sents, original_abstract_sents, selected_sents, \
                           selected_ids, original_selected_ids, p, r, acc, counter)
        if FLAGS.save_bin:
  ...
  def save_result(self, article_sents, reference_sents, selected_sents, selected_ids, gt_ids, precision, recall, acc, index):
