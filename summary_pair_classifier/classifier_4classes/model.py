...
class TextClassifier():
    ...
    def _add_placeholders(self):
        ...
        self._gen_batch = tf.placeholder(tf.int32, [hps.batch_size, None], name='gen_batch')
        self._gen_lens = tf.placeholder(tf.int32, [hps.batch_size], name='gen_lens')
        self._gen_padding_mask = tf.placeholder(tf.float32, [hps.batch_size, None], name='gen_padding_mask')
    ...
    def _make_feed_dict(self, batch):
        ...
        feed_dict[self._gen_batch] = batch['gen']
        feed_dict[self._gen_lens] = batch['gen_len']
    ...
    def _lstm_encoder(self, text, length, is_article=True, name='lstm_encoder', reuse=False):
        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope("lstm"):
                lstm_cell = tf.contrib.rnn.LSTMCell(hidden_size, state_is_tuple=True, reuse=reuse)
                lstm_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=1-hps.drop_out_rate)
            with tf.device('/gpu:0'), tf.variable_scope("embedding"):
                word_emb_W = tf.get_variable("word_emb_W", [self._vocab_size, hidden_size], "float32", random_uniform_init)
                text_embed = tf.nn.embedding_lookup(word_emb_W, text) # (batch_size, seq_len, hidden_size)

            (_, final_st) = tf.nn.dynamic_rnn(lstm_cell, text_embed, dtype=tf.float32, sequence_length=length)
            '''
            ...
            '''
        #return state_gather
        # concat the memory and hidden state
        return tf.concat(final_st, 1) # shape [batch_size, hidden_size*2]
    ...
    def build_graph(self):
        ...
        if hps.encode_method == 'cnn':
            ...
            gen_feat = self._cnn_encoder(self._gen_batch, is_article=False, reuse=True)
            ...
            gen_feat = self._lstm_encoder(self._gen_batch, self._gen_lens, is_article=False, reuse=True)
        ...
        gen_pred, _, gen_logits, _ = self._text_classifier(art_feat, gen_feat, reuse=True)
        ...
        ones = tf.ones((hps.batch_size,1))
        zeros = tf.zeros((hps.batch_size,1))
        ref_labels = tf.concat((ones, zeros, zeros, zeros), 1)
        ext_labels = tf.concat((zeros, ones, zeros, zeros), 1)
        gen_labels = tf.concat((zeros, zeros, ones, zeros), 1)
        wrong_labels = tf.concat((zeros, zeros, zeros, ones), 1)
        '''
        ref_labels = tf.concat((tf.ones((bsize,1)), tf.zeros((bsize,1)), tf.zeros((bsize,1)), tf.zeros((bsize,1))), 1)
        ext_labels = tf.concat((tf.zeros((bsize,1)), tf.ones((bsize,1)), tf.zeros((bsize,1)), tf.zeros((bsize,1))), 1)
        gen_labels = tf.concat((tf.zeros((bsize,1)), tf.zeros((bsize,1)), tf.ones((bsize,1)), tf.zeros((bsize,1))), 1)
        wrong_labels = tf.concat((tf.zeros((bsize,1)), tf.zeros((bsize,1)), tf.zeros((bsize,1)), tf.ones((bsize,1))), 1)
        '''
        ...
        gen_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=gen_logits, labels=gen_labels))
        wrong_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=wrong_logits, labels=wrong_labels))

        self.loss = ref_loss + ext_loss + gen_loss + wrong_loss
        ...
        self.gen_accu = tf.reduce_mean(tf.cast(tf.equal(gen_pred, tf.argmax(gen_labels, 1)), tf.float32))
        self.wrong_accu = tf.reduce_mean(tf.cast(tf.equal(wrong_pred, tf.argmax(wrong_labels, 1)), tf.float32))
        self.total_accu = (self.ref_accu + self.ext_accu + self.gen_accu + self.wrong_accu) / 4.0
        tf.summary.scalar('ref_accu', self.ref_accu)
        tf.summary.scalar('ext_accu', self.ext_accu)
        tf.summary.scalar('gen_accu', self.gen_accu)
    ...
    def _eval_epoch(self, test_data):
        ...
        while 1:
            ...
            gen_accu.append(results['gen_accu'])
        ...
        avg_gen_accu = sum(gen_accu)/float(len(gen_accu))
        avg_wrong_accu = sum(wrong_accu)/float(len(wrong_accu))
        avg_total_accu = sum(total_accu)/float(len(total_accu))
        return avg_loss, avg_ref_accu, avg_ext_accu, avg_gen_accu, avg_wrong_accu, avg_total_accu
        ...
        for _ in range(hps.max_iter):
            ...
            print '[Info] Train step: %d\n loss: %f\n ref accu: %f\n ext accu: %f\n gen accu: %f\n wro accu: %f\n avg accu: %f' % \
                  (cur_iter, results['loss'], results['ref_accu'], results['ext_accu'], \
                             results['gen_accu'], results['wrong_accu'], results['total_accu'])
            ...
            if cur_iter % 100 == 0:
                t0 = time.time()
                test_loss, test_ref_accu, test_ext_accu, test_gen_accu, test_wrong_accu, test_total_accu = \
                                                                             self._eval_epoch(self._test_data)
                t1 = time.time()
                print '---------------------------------------'
