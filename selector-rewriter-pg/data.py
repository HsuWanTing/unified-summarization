...
def outputids2words(id_list, vocab, article_oovs):
  ...
  for i in id_list:
    try:
      ...
      w = article_oovs[article_oov_idx] if article_oov_idx < len(article_oovs) else UNKNOWN_TOKEN
      '''
      try:
        w = article_oovs[article_oov_idx]
      except ValueError as e: # i doesn't correspond to an article oov
        w = UNKNOWN_TOKEN # in training, becuase batch size is not 1, so the oov index may out of list
...
def document2sents(document):
  """Splits document text from datafile into list of sentences.
